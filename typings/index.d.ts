...
declare module '@spikers/noblox.js' {
	// Interfaces/Types
	...
	interface NobloxOptions {
		...
		cache: {
			...
			/** Ten minute cache for a user object based on their name.*/
			UserFromName: {
				/** Default: 600 */
				expire: number | boolean;
				/** Default: false */
	...
	function getCurrentAvatar(
		option?: 'scales' | 'playerAvatarType' | 'bodyColors' | 'assets' | 'defaultShirtApplied' | 'defaultPantsApplied' | undefined,
		jar?: CookieJar,
	): Promise<AvatarInfo>;
	 ...
	 */
	function getDatastoreEntryVersion(
		universeId: number,
		datastoreName: string,
		entryKey: string,
	 ...
	 */
	function incrementDatastoreEntry(
		universeId: number,
		datastoreName: string,
		entryKey: string,
		incrementBy: number,
		scope?: string,
		robloxEntryUserIDs?: number[],
		robloxEntryAttributes?: object,
		jar?: CookieJar,
	): Promise<DatastoreEntry>;
	 ...
	 */
	function configureGamePass(
		gamePassId: number,
		name: string,
		description?: string,
		price?: number | boolean,
		icon?: string | stream.Stream,
		jar?: CookieJar,
	 ...
	 */
	function updateDeveloperProduct(
		universeId: number,
		productId: number,
		priceInRobux: number,
		name?: string,
	 ...
	 */
	function getPlayerThumbnail(
		userIds: number | number[],
		size: BodySizes | BustSizes | HeadshotSizes,
		format?: 'png' | 'jpeg',
		isCircular?: boolean,
	 ...
	 * Username is not case-sensitive.
	 */
	function getUserFromUsername<T extends string | string[]>(
		username: T,
	): T extends string ? Promise<MultiGetUserByNameResponse> : Promise<MultiGetUserByNameResponse[]>;
	...
	function getCurrentUser(
		option?: 'UserID' | 'UserName' | 'RobuxBalance' | 'ThumbnailUrl' | 'IsAnyBuildersClubMember' | 'IsPremium' | undefined,
		jar?: CookieJar,
	 ...
	 */
	function shortPoll(
		getLatest: (latest: number, event: events.EventEmitter) => Promise<GetLatestResponse>,
		delay: string | number,
		timeout?: number,
	): events.EventEmitter;
	 ...
	 */
	function onGroupTransaction(
		group: number,
		transactionType?: 'Sale' | 'Purchase' | 'AffiliateSale' | 'DevEx' | 'GroupPayout' | 'AdImpressionPayout',
		jar?: CookieJar,
	): OnTransactionEventEmitter;
